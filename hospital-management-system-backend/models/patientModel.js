...
// 1. Pre-save hook to generate PTXXXX ID automatically (Modern async way)
patientSchema.pre('save', async function () {
  if (!this.isNew) {
    return; // Exit if not a new document
  }
  const seq_value = await getNextSequenceValue('application_id');
  const padded_value = seq_value.toString().padStart(4, '0');
  this.application_id = `PT${padded_value}`;
});
